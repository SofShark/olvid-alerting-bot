export default defineEventHandler(async (event) => {
  ...
  if (method === "GET") {
    try {
      ...
      throw createError({
        statusCode: 500,
        statusMessage: error.message || "Failed to fetch alerts",
      });
    }
  }
  ...
  if (method === "POST") {
    try {
      ...
      throw createError({
        statusCode: 500,
        statusMessage: error.message || "Failed to create alert",
      });
    }
  }
  ...
  if (method === "PUT") {
    try {
      ...
      throw createError({
        statusCode: 500,
        statusMessage: error.message || "Failed to update alert",
      });
    }
  }
  ...
  if (method === "PATCH") {
    try {
      const body = await readBody(event);
      console.log(
        "📥 [PATCH /api/backend] Toggling status for #" + body.id,
        "->",
        body.status,
      );
      const id = Number(body.id);
  ...
  if (method === "DELETE") {
    try {
      ...
      throw createError({
        statusCode: 500,
        statusMessage: error.message || "Failed to delete alert",
      });
    }
  }
});
