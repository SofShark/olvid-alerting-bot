...
export const useIntervalLabel = () => {
  ...
  const intervalLabel = (
    intervalSeconds: number | undefined | null,
  ): string => {
    ...
    if (s % 86_400 === 0) {
      const d = s / 86_400;
      return t(
        d === 1 ? "editor.interval.everyDay" : "editor.interval.everyDays",
        { n: d },
      );
    }
    if (s % 3_600 === 0) {
      const h = s / 3_600;
      return t(
        h === 1 ? "editor.interval.everyHour" : "editor.interval.everyHours",
        { n: h },
      );
    }
    if (s % 60 === 0) {
      ...
      return t(
        m === 1
          ? "editor.interval.everyMinute"
          : "editor.interval.everyMinutes",
        { n: m },
      );
    }
